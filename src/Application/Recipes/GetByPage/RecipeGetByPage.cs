...
public class RecipeGetByPage
{
    ...
    public async Task<IEnumerable<Recipe>> GetRecipesAsync(RecipeGetByPageDto dto)
    {
        ...
        if (!Enum.TryParse<RecipeSortType>(dto.SortType, true, out _))
            dto = dto with { SortType = RecipeSortType.Popular.ToString() };

        return await _repo.SearchByPageAsync(dto.Page, dto.PageSize, Enum.Parse<RecipeSortType>(dto.SortType, true));
    }
}